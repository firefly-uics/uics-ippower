...
select id, CONCAT(target_type , '-' , device_ip,'-',device_type,'-',dp_paramname) as target_id, target_type, target_name,device_ip,device_name,device_type,CONCAT(device_ip , "-" , device_type, '-', dp_paramname) as device_id,dt_rtdata_table,dp_paramname from (
